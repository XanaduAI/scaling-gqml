...
# Code modified from qml-benchmarks package  https://github.com/XanaduAI/qml-benchmarks
...
def read_data(path, labels=True):
    ...
    if labels:
        ...
        X = data.iloc[:, :].values
        y = None
        # y = [None for __ in range(len(X))]
    return X, y

def construct_hyperparameter_grid(hyperparameter_settings):
    ...
    """
    hyperparams = hyperparameter_settings.keys()
    hyperparameter_grid = {}

    for hyperparam in hyperparams:
        if hyperparameter_settings[hyperparam]["type"] == "list":
            val = hyperparameter_settings[hyperparam]["val"]
            dtype = hyperparameter_settings[hyperparam]["dtype"]
            if dtype == "tuple":
