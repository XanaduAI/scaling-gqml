...
name = f'ising_network_{N}_nodes_T_{T}'
if not os.path.exists(current_dir+'/'+name):
    os.makedirs(current_dir+'/'+name)

settings = {'n_nodes': N, 'm': m, 'T': T, 'burn_in': burn_in, 'num_samples_per_chain': num_samples_per_chain,
...
plt.show()
plt.savefig(current_dir+'/'+ name + '/' +name+'_network.png')
