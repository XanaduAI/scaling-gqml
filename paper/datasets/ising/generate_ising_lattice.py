...
# Generates a dataset by sampling from the ising distirbution of a 2d square lattice
...
width = 3  # Grid dimensions
T =2.5 #Temperature
num_chains = 8 #number of independent MCMC chains
num_samples_per_chain = 2000
burn_in = 1000 #point at which to start accepting samples as valid on the chain
num_samples_train = 1000 #number of training samples to generate
num_samples_test = 1000 #used for witness points and energy/magnetisation moments
random = False #if False, weights are all set to 1. If True, weights are set to random positive numbers.
...
b = np.zeros(N) #change if you want a bias
...
#save as binary data
np.savetxt(current_dir + '/'+ name + '/' +name+'_train.csv', (samples_train+1)//2, delimiter=",", fmt='%d')
np.savetxt(current_dir + '/'+ name + '/' +name+'_test.csv', (samples_test+1)//2, delimiter=",", fmt='%d')
...
plt.savefig(current_dir+'/'+ name + '/' +name+'_examples.png')
plt.show()
...
np.savetxt(current_dir+'/'+ name + '/' +name+'_moments.csv', np.array([mom_en+mom_mag]), delimiter=",", header = header)

#plot the energy and magnetization distributions
plt.hist(energies_test, bins=15)
plt.title('energies test')
plt.savefig(current_dir+'/'+ name + '/' +name+'_en_dist_test.png')
plt.show()
plt.hist(energies_train, bins=15)
plt.title('energies train')
plt.savefig(current_dir+'/'+ name + '/' +name+'_en_dist_train.png')
plt.show()
plt.hist(magnetizations_test, bins=15)
plt.title('magnetizations test')
plt.savefig(current_dir+'/'+ name + '/' +name+'_mag_dist_test.png')
plt.show()
plt.hist(magnetizations_train, bins=15)
plt.title('magnetizations train')
plt.savefig(current_dir+'/'+ name + '/' +name+'_mag_dist_train.png')
plt.show()
